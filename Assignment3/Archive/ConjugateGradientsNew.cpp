...
extern Timer timerCombinedKernel;
extern Timer timerLaplacianAndInnerProduct;
extern Timer timerCombinedSaxpyNorm;

void ConjugateGradientsNew(
    ...
    const bool writeIterations)
{
    ...
    timerCombinedKernel.Restart(); float nu = CombinedKernel(x, f, p, r, z); timerCombinedKernel.Pause();
    ...
    for(int k=0;;k++)
    {
        ...
        timerCombinedSaxpyNorm.Restart(); nu = CombinedSaxpyNorm(r, z, r, r, -alpha); timerCombinedSaxpyNorm.Pause();
        ...
        // Algorithm : Line 16
        timerSaxpy.Restart(); Saxpy(p, x, x, alpha); timerSaxpy.Pause();
        timerSaxpy.Restart(); Saxpy(p, r, p, beta); timerSaxpy.Pause();
